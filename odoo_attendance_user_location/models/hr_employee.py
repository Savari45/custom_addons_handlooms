...
class HrEmployee(models.AbstractModel):
    ...
    def _attendance_action_change(self, latitude=False, longitude=False):
        """ Check In/Check Out action with geolocation support """
        self.ensure_one()
        action_date = fields.Datetime.now()
        ...
        if self.attendance_state != 'checked_in':
            vals = {
                ...
                'checkin_address': location.address if location else False,
                'checkin_location': f"https://www.google.com/maps/search/?api=1&query={latitude},{longitude}" if latitude and longitude else False,
            }
            return self.env['hr.attendance'].create(vals)
        ...
        if attendance:
            attendance.write({
                ...
                'checkout_address': location.address if location else False,
                'checkout_location': f"https://www.google.com/maps/search/?api=1&query={latitude},{longitude}" if latitude and longitude else False,
